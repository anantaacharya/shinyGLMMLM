...
model1<-c("PC","Q")
   ...
   # m2<-subset(masterglmmlm.test,blupL %in% tr1 & GLMMLM %in% gl1 & model %in% model1 & df %in% df1,drop=FALSE)
    ...
    #print(p)
     ...
     q<-ggplot(aes(pos, -log10(p), colour=interaction(GLMMLM, model,blupL)), data=plot.temp())+geom_point(aes(shape=df), size=4, solid=F)+scale_shape_manual(values=0:5)+facet_wrap(~Locus)
